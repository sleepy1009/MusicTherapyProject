...
class Command(BaseCommand):
    help = 'Lọc, làm sạch (Diệt nhạc địa phương) và nhập dữ liệu vào Database'
    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            ...
            # 3. LỌC NÂNG CAO: DIỆT TẬN GỐC NHẠC ĐỊA PHƯƠNG (Giữ K-Pop, J-Pop)
            regional_genres = [
            ...
            # BƯỚC 3.1: Tìm Tên và Nghệ sĩ của TẤT CẢ các bài hát bị dính tag đen
            bad_tracks = df[df['track_genre'].isin(regional_genres)][['track_name', 'artists']].drop_duplicates()
            bad_tracks['is_bad'] = True # Đánh dấu cờ
            
            # BƯỚC 3.2: Gộp (Merge) đối chiếu để loại bỏ
            # Các bài Pop, Acoustic nếu trùng tên/nghệ sĩ với bad_tracks sẽ bị gắn cờ is_bad = True
            df = df.merge(bad_tracks, on=['track_name', 'artists'], how='left')
            
            # Chỉ giữ lại những bài không bị gắn cờ (is_bad bị null)
            df = df[df['is_bad'].isnull()]
            df = df.drop(columns=['is_bad'])
            
            # 4. CHỐT HẠ: Loại bỏ trùng lặp ID bài hát
            ...
            for index, row in df.iterrows():
                ...
                # Cứ gom đủ 5000 bài thì nhét vào DB 1 lần
            ...
            self.stdout.write(self.style.SUCCESS('✅ Nhập dữ liệu THÀNH CÔNG! Database đã SẠCH BÓNG NHẠC ẤN ĐỘ!'))
