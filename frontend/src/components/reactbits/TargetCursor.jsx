...
const TargetCursor = ({
  ...
  useEffect(() => {
    ...
    const checkOutOfTarget = e => {
      const { clientX, clientY } = e;
      const el = document.elementFromPoint(clientX, clientY);
      if (
        isActiveRef.current &&
        (!el || !el.closest(targetSelector))
      ) {
        if (currentLeaveHandler) {
          currentLeaveHandler();
        }
    ...
    window.addEventListener('mousemove', checkOutOfTarget);
    ...
    return () => {
      ...
      window.removeEventListener('mousemove', checkOutOfTarget);
      if (activeTarget) {
        cleanupTarget(activeTarget);
      }
      spinTl.current?.kill();
